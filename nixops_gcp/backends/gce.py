        ...
        def parse_block_device(diskConfig: GCEDiskOptions) -> Dict:
            ...
            if not (result["disk"] or result["disk_name"]):
                raise Exception(
                    ...
                    "external disk name to mount or a disk name to create".format(
                        self.machine_name
                    )
                )
            return result
...
class GCEState(MachineState[GCEDefinition], ResourceState):
    ...
    def show_type(self):
        ...
        if self.region:
            s = "{0} [{1}; {2}]".format(s, self.region, self.instance_type)
        return s
    ...
    def _delete_volume(self, volume_id, region, allow_keep=False):
        if not self.depl.logger.confirm(
            "are you sure you want to destroy GCE disk '{0}'?".format(volume_id)
        ):
            if allow_keep:
                return
            else:
        ...
        if check:
            try:
                ...
                if self.vm_id:
                    ...
                    self.handle_changed_property(
                        "region", node.extra["zone"].name, can_fix=False
                    )
                    self.handle_changed_property(
                        "preemptible",
                        node.extra["scheduling"]["preemptible"],
                        can_fix=False,
                    )

                    # a bit hacky but should work
                    network_name = node.extra["networkInterfaces"][0]["network"].split(
                        "/"
                    )[-1]
            ...
            for k, v in defn.block_device_mapping.items():
                ...
                try:
                    ...
                    if v["disk"]:
                        raise Exception(
                            "external disk '{0}' is required but doesn't exist".format(
                                disk_name
                            )
                        )
                    if k in self.block_device_mapping and v["disk_name"]:
                        self.warn(
                            "disk '{0}' is supposed to exist, but is missing; will recreate...".format(
                                disk_name
                            )
                        )
        ...
        for k, v in defn.block_device_mapping.items():
            ...
            if v["disk"] is None:
                ...
                try:
                    ...
                    # in doesn't exist.
                    raise Exception(
                        "tried creating a disk in nonexistent "
                        "region %r" % v["region"]
                    )
    ...
    def create_node(self, defn):
        ...
        if not self.vm_id:
            ...
            boot_disk = next(
                (
                    v
                    for v in defn.block_device_mapping.values()
            ...
            try:
                ...
                node = self.connect().create_node(
                    ...
                    external_ip=(
                        self.connect().ex_get_address(defn.ipAddress)
                        if defn.ipAddress
                        else "ephemeral"
                    ),
        ...
        if self.email != defn.email or self.scopes != defn.scopes:
            ...
            if defn.scopes != []:
                service_account["scopes"] = defn.scopes
            self.connect().connection.async_request(
                request, method="POST", data=service_account
            )
        ...
        if self.labels != defn.labels:
            ...
            node = self.node()
            labels_request = "/zones/%s/instances/%s" % (
                node.extra["zone"].name,
                node.name,
            )
            response = (
                self.connect().connection.request(labels_request, method="GET").object
            )
            body = {
                "labels": defn.labels,
                "labelFingerprint": response["labelFingerprint"],
            }
            request = "/zones/%s/instances/%s/setLabels" % (
                node.extra["zone"].name,
                node.name,
            )
            ...
            for k, v in self.block_device_mapping.items():
                ...
                if not (
                    ("disk" in disk_name or "part" in disk_name)
                    and (disk_name.startswith(node.name))
                ):
                    continue
                disk_labels_request = "/zones/%s/disks/%s" % (
                    node.extra["zone"].name,
                    disk_name,
                )
                response = (
                    self.connect()
                    .connection.request(disk_labels_request, method="GET")
                    .object
                )
                body = {
                    "labels": defn.labels,
                ...
                request = "/zones/%s/disks/%s/setLabels" % (
                    node.extra["zone"].name,
                    disk_name,
                )
                self.connect().connection.async_request(
            ...
            if v.get("needsAttach", False) and defn_v:
                ...
                if not v.get("bootDisk", False):
                    self.connect().attach_volume(
                        self.node(),
                        self.connect().ex_get_volume(disk_volume, disk_region),
                        device=disk_name,
        ...
        if self.metadata != defn.metadata:
            ...
            self.connect().connection.async_request(
                request, method="POST", data=metadata_data
            )
        ...
        if self.public_ipv4 is None:
            self.log(
                "attaching public IP address {0}".format(
                    defn.ipAddress or "[Ephemeral]"
                )
            )
            self.connect().connection.async_request(
                "/zones/{0}/instances/{1}/addAccessConfig?networkInterface=nic0".format(
                    self.region, self.machine_name
                ),
                method="POST",
                data={
                    ...
                    "type": "ONE_TO_ONE_NAT",
                    "name": "External NAT",
                    "natIP": self.connect().ex_get_address(defn.ipAddress).address
                    if defn.ipAddress
                    else None,
                },
            )
        ...
        if (
            self.automatic_restart != defn.automatic_restart
            or self.on_host_maintenance != defn.on_host_maintenance
        ):
    ...
    def start(self):
        ...
        if not self.vm_id and self.block_device_mapping:
            ...
            if prev_public_ipv4 != self.public_ipv4:
                self.warn(
                    "Public IP address has changed from {0} to {1}, "
                    "you may need to run 'nixops deploy'".format(
                        prev_public_ipv4, self.public_ipv4
                    )
                )
            if prev_private_ipv4 != self.private_ipv4:
                self.warn(
                    "Private IP address has changed from {0} to {1}, "
                    "you may need to run 'nixops deploy'".format(
                        prev_private_ipv4, self.private_ipv4
                    )
                )
            ...
            if k not in defn.block_device_mapping:
                ...
                try:
                    if not v.get("needsAttach", False):
                        ...
                        volume = self.connect().ex_get_volume(
                            disk_name, v.get("region", None)
                        )
                    ...
                    self.warn(
                        "GCE disk '{0}' seems to have been already destroyed".format(
                            disk_name
                        )
                    )
    ...
    def get_console_output(self):
        ...
        if node.state == NodeState.TERMINATED:
            raise Exception(
                "cannot get console output of a state=TERMINATED machine '{0}'".format(
                    self.name
                )
            )
        request = "/zones/%s/instances/%s/serialPort" % (
            node.extra["zone"].name,
            node.name,
        )
        return (
            self.connect().connection.request(request, method="GET").object["contents"]
        )
    ...
    def _check(self, res):
        try:
            ...
            res.exists = True
            res.is_up = (
                node.state == NodeState.RUNNING or node.state == NodeState.REBOOTING
            )
            if node.state == NodeState.REBOOTING or node.state == NodeState.PENDING:
                self.state = self.STARTING
            if node.state == NodeState.STOPPED or node.state == NodeState.TERMINATED:
                self.state = self.STOPPED
            ...
            if node.state == NodeState.RUNNING:
                ...
                for v in self.block_device_mapping.values():
                    disk_name = v["disk_name"] or v["disk"]
                    if all(
                        d.get("deviceName", None) != disk_name
                        for d in node.extra["disks"]
                    ):
                        res.disks_ok = False
                        ...
                        # been destroyed.
                        try:
                            self.connect().ex_get_volume(
                                disk_name, v.get("region", None)
                            )
                        except libcloud.common.google.ResourceNotFoundError:
                            res.messages.append(
                                "disk {0} is destroyed".format(disk_name)
                            )
                self.handle_changed_property(
                    "public_ipv4",
                    node.public_ips[0] if node.public_ips else None,
    ...
    def create_after(self, resources, defn):
        ...
        return {
            r
            for r in resources
            if isinstance(r, nixops_gcp.resources.gce_static_ip.GCEStaticIPState)
            or isinstance(r, nixops_gcp.resources.gce_disk.GCEDiskState)
            or isinstance(r, nixops_gcp.resources.gce_image.GCEImageState)
            or isinstance(r, nixops_gcp.resources.gce_network.GCENetworkState)
        }

    def backup(self, defn, backup_id, devices=[]):
        self.log("backing up {0} using ID '{1}'".format(self.full_name, backup_id))

        if sorted(defn.block_device_mapping.keys()) != sorted(
            self.block_device_mapping.keys()
        ):
            self.warn(
                "the list of disks currently deployed doesn't match the current deployment"
        ...
        for k, v in self.block_device_mapping.items():
            ...
            if devices == [] or k in devices or disk_name in devices:
                ...
                self.log(
                    "initiating snapshotting of disk '{0}': '{1}'".format(
                ...
                self.connect().connection.request(
                    ...
                    data={
                        "name": snapshot_name,
                        "description": "backup of disk {0} attached to {1}".format(
                            volume.name, self.machine_name
                        ),
                    },
                )
                ...
                if defn.labels:
                    ...
                    self.connect().connection.request(
                        ...
                        data={
                            "labels": defn.labels,
                            "labelFingerprint": self.connect()
                            .connection.request(
                                "/global/snapshots/{0}".format(snapshot_name),
                                method="GET",
                            )
        ...
        while True:
            try:
                ...
                if snapshot.status in "READY" "CREATING" "UPLOADING":
                    ...
                    raise Exception(
                        "snapshot '{0}' is in an unexpected state {1}".format(
        ...
        for k, v in self.block_device_mapping.items():
            ...
            if s_id and (devices == [] or k in devices or disk_name in devices):
                ...
                try:
                    ...
                    self.warn(
                        "disk {0} seems to have been destroyed already".format(
                            disk_name
                        )
                    )

                self.log(
                    "creating disk {0} from snapshot '{1}'".format(disk_name, s_id)
                )
        ...
        for b_id, snapshots in self.backups.items():
            ...
            for k, v in self.block_device_mapping.items():
                ...
                if not disk_name in snapshots.keys():
                    ...
                    info.append(
                        "{0} - {1} - not available in backup".format(
                            self.name, disk_name
                        )
                    )
                else:
                    ...
                    try:
                        ...
                        info.append(
                            "{0} - {1} - {2} - snapshot has disappeared".format(
                                self.name, disk_name, snapshot_id
                            )
                        )
                        backup_status = "unavailable"
            for d_name, s_id in snapshots.items():
                if not any(
                    d_name == v["disk_name"] or d_name == v["disk"]
                    for k, v in self.block_device_mapping.items()
                ):
                    info.append(
                        "{0} - {1} - {2} - a snapshot of a disk that is not or no longer deployed".format(
                            self.name, d_name, s_id
                        )
                    )
    ...
    def get_ssh_name(self):
        if not self.public_ipv4:
            raise Exception(
                "{0} does not have a public IPv4 address (yet)".format(self.full_name)
            )
