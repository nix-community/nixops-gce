    ...
    def __init__(self, xml):
        ...
        def parse_lifecycle(x):
            ...
            if created_before:
                ...
                if m:
                    normalized_created_before = "{0[0]:0>4}-{0[1]:0>2}-{0[2]:0>2}".format(
                        m.groups()
                    )
                else:
                    raise Exception(
        ...
        if any(
            ...
            raise Exception(
                "Bucket '{0}' object lifecycle management "
...
class GSEBucketState(ResourceState):
    ...
    def bucket_resource(self, defn):
        return {
            ...
            if defn.log_bucket is not None
            else {},
    ...
    def bucket(self):
        return (
            self.connect()
            .request("/{0}?projection=full".format(self.bucket_name), method="GET")
            .object
        )
    ...
    def create(self, defn, check, allow_reboot, allow_recreate):
            ...
            try:
                ...
                if self.state == self.UP:
                    ...
                    self.handle_changed_property(
                        "storage_class", b["storageClass"], can_fix=False
                    )

                    self.handle_changed_property(
                        "log_bucket", b.get("logging", {}).get("logBucket", None)
                    )
                    self.handle_changed_property(
                        "log_object_prefix",
                    ...
                    self.handle_changed_property(
                        "cors", actual_cors, property_name="CORS config"
                    )
                        ...
                        [
                            {
                                ...
                                "created_before": r.get("condition", {}).get(
                                    "createdBefore", None
                                ),
                                "number_of_newer_versions": r.get("condition", {}).get(
                    ...
                    self.handle_changed_property(
                        "lifecycle", actual_lifecycle, property_name="lifecycle config"
                    )

                else:
                    self.warn_not_supposed_to_exist(
                        valuable_resource=True, valuable_data=True
                    )
                    if self.depl.logger.confirm(
                        "are you sure you want to destroy the existing {0}?".format(
                            self.full_name
                        )
                    ):
                        self.log("destroying...")
        ...
        if self.state != self.UP:
            ...
            try:
                ...
                if (
                    e.value.get("message", None)
                    == "You already own this bucket. Please select another name."
                ):
                    raise Exception(
                        "tried creating a GSE bucket that already exists; "
