...
class GCETargetPoolDefinition(ResourceDefinition):
    ...
    def __init__(self, name, config):
        ...
        if not all(m for m in self.machines):
            raise Exception(
                "target pool machine specification must be either a NixOps "
...
class GCETargetPoolState(ResourceState):
    ...
    def create(self, defn, check, allow_reboot, allow_recreate):
                ...
                if self.state == self.UP:
                    ...
                    if machines_state != normalized_machines:
                        if normalized_machines - machines_state:
                            self.warn(
                                "{0} contains unexpected machines: {1}".format(
                                    self.full_name,
                                    list(normalized_machines - machines_state),
                                )
                            )
                        if machines_state - normalized_machines:
                            self.warn(
                                "{0} is missing machines: {1}".format(
                                    self.full_name,
                                    list(machines_state - normalized_machines),
                                )
                            )
        ...
        if self.state != self.UP:
            ...
            try:
                tp = self.connect().ex_create_targetpool(
                    defn.targetpool_name,
                    region=defn.region,
                    healthchecks=([defn.health_check] if defn.health_check else None),
                )
            except libcloud.common.google.ResourceExistsError:
                raise Exception(
                    "tried creating a target pool that already exists; "
    ...
    def create_after(self, resources, defn):
        return {r for r in resources if isinstance(r, GCEHTTPHealthCheckState)}
